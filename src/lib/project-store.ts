...
const SEARCH_API_CONFIG_KEY = "searchApiConfig"

export async function saveSearchApiConfig(config: SearchApiConfig): Promise<void> {
  await saveSecureConfig(SEARCH_API_CONFIG_KEY, config)
}

export async function loadSearchApiConfig(): Promise<SearchApiConfig | null> {
  return loadSecureConfig<SearchApiConfig>(SEARCH_API_CONFIG_KEY)
}
